...
| The Quick Look preview steals keyboard focus, so arrow keys scroll the document instead of moving the Finder selection | Issue → PR | **submitted** — upstream's own issue [#292](https://github.com/pluk-inc/markdown-preview/issues/292), [PR #370](https://github.com/pluk-inc/markdown-preview/pull/370), open. **Carried here** from 2026-09-11: we ship what we build rather than waiting for review. The commit is the same patch as the PR branch, so the next sync sees it on both sides |
